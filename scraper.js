...
async function scrapeAndCollectLinks(page = 1, collectedLinks = new Set()) {
  try {
    ...
    $('ul.c-jobListView__list').find('li').each((index, element) => {
      const link = $(element).find('div.o-listView__itemInfo > a').attr('href');
      if (link) {
...
async function scrapeLink(link) {
  try {
    ...
    const jobDetails = $('section:not([class]) ul.o-listView__list.c-jobListView__list li.o-listView__item').map((index, element) => {
      const $element = $(element);
      // Extract company name
      const companyName = $element.find('div > div.o-listView__itemInfo > ul > li:nth-child(1) > span').text().trim();
      // Extract position
      const position = $element.find('div > div.o-listView__itemInfo > h2 > a').text().trim();
      // Extract link
      const jobLink = $element.find('div > div.o-listView__itemInfo > h2 > a').attr('href');
      // Check if position name includes any of the keywords
      const filtered = keywords.some(keyword => position.toLowerCase().includes(keyword));
...
async function sendJobDetailsToTelegram(jobDetails) {
  ...
  try {
    if (jobDetails.length > 0) {
      ...
      const jobDetailsText = chunksOfJobs.map(jobDetails => messageHeader + nowDate + jobDetails.map(job => `Company Name: ${job.companyName}\nPosition: [${job.position}](${job.jobLink})\n`).join('\n\n'));


      // Send each chunk as a separate message
      for (const chunk of jobDetailsText) {
        // Send message to Telegram chat
        await bot.telegram.sendMessage(process.env.TELEGRAM_CHAT_ID, chunk, { parse_mode: 'Markdown' });
      }
